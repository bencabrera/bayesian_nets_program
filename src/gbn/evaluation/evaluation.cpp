...
namespace {
	MatrixPtr evaluate_wire_structure(const GBN& gbn, WireStructure wire_structure, std::size_t n_inputs, std::size_t n_outputs, std::vector<Vertex> inside_vertices)
	{
		auto& g = gbn.graph;

		auto& wires = wire_structure.wires;
		MatrixPtr m = std::make_shared<DynamicMatrix>(n_inputs, n_outputs);
...
MatrixPtr evaluate_gbn(const GBN& gbn)
{
	...
	auto [input_ports, output_ports] = build_inputs_outputs_for_gbn(gbn);
	auto wire_structure = build_wire_structure_for_gbn(gbn);

	return evaluate_wire_structure(gbn, wire_structure, input_ports.size(), output_ports.size(), ::inside_vertices(gbn));
}


MatrixPtr evaluate_vertices(const GBN& gbn, std::vector<Vertex> inside_vertices)
{
	// TODO: check that path closed
	// TODO: check that vertices does not contain input or output vertices
	// TODO: for efficiency move all F matrices to front and multiply it into another matrix
	...
	auto wire_structure = build_wire_structure_for_vertices(gbn, inside_vertices);

	return evaluate_wire_structure(gbn, wire_structure, input_ports.size(), output_ports.size(), inside_vertices);
}

MatrixPtr evaluate(const GBN& gbn, std::vector<Vertex> inside_vertices)
{
	if(inside_vertices.empty())
		return evaluate_gbn(gbn);	
	else
		return evaluate_vertices(gbn, inside_vertices);	
}
