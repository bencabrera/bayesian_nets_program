...
{
	auto ports = generate_random_ports(n, m, n_inside_vertices, mt, params);
	auto input_to_output_map = randomly_connect_ports(n, m, n_inside_vertices, mt, params, ports);
...
// // TODO: this is kind of a hacky solution and should propably be overhauled...
// GBN generate_random_gbn(std::size_t n, std::size_t m, std::size_t n_inside_vertices, std::mt19937& mt, RandomGBNParams params)
// {
// GBN gbn(n,m,n_inside_vertices);
...
// auto precessor_ports = draw_random_elements<std::vector<Port>::iterator, Port>(it, previous_output_ports.end(), n_precessors, mt);
...
// std::cout << "fuenf" << std::endl;

// // connect output ports
// auto precessor_ports_for_outputs = draw_random_elements<std::vector<Port>::iterator, Port>(non_input_ports.begin(), non_input_ports.end(), m-input_ports_without.size(), mt);
// if(precessor_ports_for_outputs.empty())
// throw std::logic_error("'precessor_ports_for_outputs' in 'generate_random_gbn' is empty.");
...
// // auto& port = precessor_ports_for_outputs.at(i_port % precessor_ports_for_outputs.size());
// auto& port = precessor_ports_for_outputs.at(i_port % precessor_ports_for_outputs.size());
